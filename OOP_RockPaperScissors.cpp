...
public:
  ...
  bool fight(Tool &opponent) override{
    ...
    if (opponent.getType() == 's'){
      myStrength *= 2;} //Rock beats scissor
    else if (opponent.getType() == 'p'){
      myStrength /= 2;} //Rock loses to paper
...
public:
  ...
  bool fight(Tool &opponent) override{
    ...
    if (opponent.getType() == 'r'){
      myStrength *= 2;} //Paper beats rock
    else if (opponent.getType() == 's'){
      myStrength /= 2;} //Paper loses to scissors
...
public:
  ...
  bool fight(Tool &opponent) override{
    ...
    if (opponent.getType() == 'p'){
      myStrength *= 2;} //Scissors beats paper
    else if (opponent.getType() == 'r'){
      myStrength /= 2;} //Scissors loses to rock
